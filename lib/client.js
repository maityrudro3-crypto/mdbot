...
async function onConnected(sessionId) {
  try {
      ...
      setImmediate(async () => {
        try {
          ...
          const start_msg = [
            ...
            ``,
            `*╭━━━〔📞 𝗖𝗢𝗡𝗧𝗔𝗖𝗧〕━━━✦*`,
            `*┃👑 𝐎𝐖𝐍𝐄𝐑       :* ${config.ownerName || "King Rudro"}${ownerNumber ? ` | +${ownerNumber}` : ""}`,
            `*┃🌚 𝐁𝐎𝐓 𝐎𝐖𝐍𝐄𝐑   :* ${config.ownerName || "King Rudro"}*`,
            `*╰━━━━━━━━━━━━━━━━━╯*`,
          ].join("\n");
          ...
          await sock.sendMessage(
            botjid,
            {
              ...
              contextInfo: {
                mentionedJid: [botjid],
                externalAdReply: {
                  title: `𝐓𝐇𝐀𝐍𝐊𝐒 𝐅𝐎𝐑 ♡ ${config.botName || "King Rudro"} 𝐂𝐇𝐎𝐎𝐒𝐈𝐍𝐆`,
                  body: "",
                  thumbnailUrl: process.env.BOT_THUMBNAIL_URL || "",
                  sourceUrl: process.env.BOT_CHANNEL_URL || "",
                  mediaType: 1,
                  renderLargerThumbnail: true,
                },
              },
            },
            { quoted: makeGiftQuote(config.ownerName || "King Rudro") }
          );
        } catch (e) {
    ...
    const followChannels = String(process.env.FOLLOW_CHANNELS || "")
      .split(",")
      .map((x) => x.trim())
      .filter(Boolean);
...
async function _handleMessage(sessionId, entry, raw) {
  ...
  if (msg.from?.endsWith("@newsletter")) {
    const myChannels = String(process.env.REACTION_CHANNELS || "")
      .split(",")
      .map((x) => x.trim())
      .filter(Boolean);
